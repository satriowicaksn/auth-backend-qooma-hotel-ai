...
> **Scope reminder**: repo ini = service **Auth & Identity** (lihat [docs/SERVICE-CHARTER.md](./docs/SERVICE-CHARTER.md) + [ADR-0008](./docs/decisions/0008-repo-scope-auth.md)). Hotel Core (CRM), AI Orchestration, dan Integration hidup di repo terpisah; slot di sini hanya menggarap domain Auth (login/sessions/users/hotels/tiers/RBAC). **Canonical implementation scope**: [docs/spec/MVP-AUTH-FIRST.md](./docs/spec/MVP-AUTH-FIRST.md).

| Slot | Nama   | Owns (default domain — per `docs/SERVICE-CHARTER.md §3`)                                                                                                                |
| ---- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| A    | Nathan | Foundation: core/, plugins, entrypoints, migrations (incl. tier seed + first super_admin), tenant-guard middleware, seed-super-admin CLI, ADR-touching tasks            |
| B    | Nanak  | Auth core: `/api/auth/*` (login/logout/refresh/me/password), session/JWT/CSRF plumbing, password rotation enforcement, `/api/users` (gm_admin scope) generate-and-return |
| C    | Satrio | Auth admin surface: `/api/admin/users` (super_admin), `/api/admin/hotels` (atomic GM-create), `/api/admin/tiers` (read), `/api/hotels/me`, `/api/settings/hotel`, suspend-cascade |

> Domain split di atas adalah **default starter** — Parent PM boleh re-balance per gate, tapi tetap pertahankan rule "shared-infra ke Dev A supaya B & C tidak saling tunggu" (lihat pola di frontend `WAVE-B-KICKOFF.md`). **Canonical task source**: `docs/spec/MVP-AUTH-FIRST.md` §1 (10-row scope table). Parent PM translate row-row tersebut jadi T## di `PM-STATUS-PARENT.md §1` lalu assign ke slot per table di atas.
