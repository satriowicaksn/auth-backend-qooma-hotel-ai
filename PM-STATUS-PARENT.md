...
| --- | -------------------------------------------------------------------------------------------------------- | ---- | ----- | ----------------------------------------------- | ----------- | ----- |
| T01 | pnpm install verify + `make check` green on bootstrap                                                    | A    | exec-A | `assigned · READY-FULL (Slot A online 2026-06-30)` | —           | Spec: pre-G1 foundation (no §1 row — environment task). Deps: none. ADR-0002 (pnpm), 0003 (TS strict + ESLint). Gate: **G1**. **Slot A online (Nathan)** — ASSIGNMENT issued PM-STATUS-A §2; verify on `feat/auth-core` (integration branch). |
...
| T03 | Tiers seed (4 rows: lite/professional/luxury/enterprise per config table)                                | A    | exec-A | `assigned · BLOCKED-until T01 + adopt-T02 (Slot A online)` | —           | Spec: `MVP-AUTH-FIRST §3` step 1 + `01-auth-identity §1.4` Tier shape (outbound_quota / agent_cap / user_cap / department_cap / features JSONB). Deps: T01, T02. ADR-0007. Gate: **G1**. Idempotent `prisma/seeds/` invocation — Slot A's call. **Sequencing (PM A 2026-06-30)**: branch from `feat/auth-core` (T02 migration lives there, unmerged); DoD §2 PM-STATUS-A. |
