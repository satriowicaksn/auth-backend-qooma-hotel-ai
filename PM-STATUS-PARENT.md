...
- **Day**: H0 / TBD (PO set timeline — bootstrap committed 2026-06-27)
- **Phase**: Bootstrap / pre-T01 — repo just forked from `core-backend-qooma-hotel-ai` infra + adapted to Auth scope.
- **Active gate**: G1 — Boilerplate ready (kriteria default `PM-AGENT.md §5`; PO konfirmasi). Includes: `make check` green, `make start` jalan, `seed-super-admin` CLI berfungsi, tier seed migration up.
- **Active devs**: A (Nathan) · B (Nanak) · C (Satrio) — semua belum claim task.
- **Progress (global)**: 0 / 10 MVP scope items approved (10-row scope table di `docs/spec/MVP-AUTH-FIRST.md §1`).

> **Canonical task source**: `docs/spec/MVP-AUTH-FIRST.md §1` (10-row endpoint scope table). Parent PM translate row-row menjadi T## entries di §1 di bawah, lalu route ke slot per `docs/SERVICE-CHARTER.md §3`. Backend belum punya `docs/DEVELOPMENT-PLAN.md` formal — MVP-AUTH-FIRST.md adalah source-of-truth Phase 1.
>
> **Pre-G1 bootstrap tasks** (Parent PM author di §1 saat first session):
> - T01: pnpm install + `make check` green on bootstrap (Dev A)
> - T02: `prisma migrate dev` initial migration generated dari schema (Dev A)
> - T03: `tiers` seed migration (4 rows: lite/professional/luxury/enterprise dengan config per spec §1.4) (Dev A)
> - T04: `seed-super-admin` CLI per `MVP-AUTH-FIRST.md §3` (Dev A)
> - T05+: Endpoint implementation per MVP-AUTH-FIRST §1 row 1-10, routed per SERVICE-CHARTER §3.
