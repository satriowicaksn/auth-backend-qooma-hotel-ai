...
## Q-CONTRACT-07 — Integrations endpoints
...
**FE assumed**: `GET /api/settings/billing` returns full `Billing` object with `tier`, `outbound_quota` (prepaid balance), `active_agents`, `active_users_count`, `extra_addons`, `invoices`, `daily_brief_pdf_url_latest`. `POST /api/billing/outbound-topup` returns `pending_confirmation`.

**Backend needs to confirm**:

- Low-balance alert thresholds `[20, 5]` percent **remaining** (prepaid balance — no 80/100-used thresholds).
- Prepaid balance has NO monthly reset — it refills only on top-up purchase (S=3000, M=7000, L=14000 messages, tier-independent).
...
**FE assumed**: `tiers` lookup table in Auth's DB (PO ruling H11 — "Tier exists in DB"). Read endpoint `GET /api/admin/tiers` returns all 4 rows + per-tier config (`agent_cap` [2/4/6/-1, total incl Receptionist], `user_cap`, `department_cap`, `features` JSONB, `is_custom` for enterprise; NO `outbound_quota_monthly` and NO `agent_minimum` — outbound is prepaid top-up and there is no minimum-agent floor). Super_admin-only read in MVP. `hotels.tier_id` FK → `tiers.id`. Existing `AdminHotel.tier` + `/api/hotels/me { tier }` responses keep the flat `tier: 'luxury'` string (backend joins `tiers.name` on read) for FE compatibility. Full shape: `01-auth-identity.md` §1.4 + API-CONTRACT §2.1b.

**Backend needs to confirm**:

- The 4 tier rows + the per-tier config values (FE recommendation in `01-auth-identity.md` §1.4 mirrors CLAUDE.md §1 tier matrix — adapt if PO directs different caps).
...
- `is_custom: true` for enterprise — should enterprise allow per-tenant override columns on the `hotels` row (e.g. `hotels.agent_cap_override`)? Out of MVP per PO; flag if you ship later.
...
### Q-OPS-03 — Outbound dispatch ownership

Should outbound WA dispatch live in Integration service (current proposal) or as a worker in Hotel Core that calls Integration's adapter? FE doesn't care; backend's call based on team org.
