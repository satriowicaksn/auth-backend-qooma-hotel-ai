...
| `ota_mailbox_state`                                      | Integration | opaque `hotel_id`                                                                                             |
...
### 4.3 Hotel Core ↔ Integration (separate DB recommended)

- `outbound_dispatch_queue.guest_id` → Hotel Core's `guests.id`. Opaque.
- `outbound_dispatch_queue.hotel_id` → Hotel Core's `hotels.id`. Opaque.
- `webhook_events` carries the raw payload — no need to FK into Hotel Core; it's an audit log.

**No special sync needed** — Integration writes and reads its own queue; Hotel Core just sees the side effects (new tickets, new notifications, updated quota meter).
