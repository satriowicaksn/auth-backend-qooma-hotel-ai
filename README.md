# Qooma Auth & Identity — Backend Service

> Service **Auth & Identity** di ekosistem **Qooma** (microservices) — identity + tenancy + tier boundary. Owns: login/sessions/users (semua role), `hotels` (tenant entity), `tiers` (catalog), RBAC + super_admin all-access. Per H11 2026-06-27 PO ruling, Auth grew beyond pure identity untuk absorb hotel tenant + tier catalog dari sibling Hotel Core service. Repo ini fork dari boilerplate Qooma (`@qooma/backend-service-template`); pattern reusable (`_template/`, `MODULE_TEMPLATE.md`, ADR-0001..0007) tetap.
>
> Charter lengkap: **[docs/SERVICE-CHARTER.md](./docs/SERVICE-CHARTER.md)** · Ratifikasi: **[ADR-0008](./docs/decisions/0008-repo-scope-auth.md)**.
>
> **⭐ Canonical implementation scope**: **[docs/spec/MVP-AUTH-FIRST.md](./docs/spec/MVP-AUTH-FIRST.md)** — start here. Backed by `docs/spec/01-auth-identity.md` (full surface), `docs/spec/data-model.md` (cross-service ERD), `docs/spec/API-CONTRACT.md` (canonical contract), `docs/spec/open-questions.md` (Q-CONTRACT-22/23/24 + Q-OPS-01 PO-ratified H11).
>
> Sibling services: **Hotel Core** (CRM operational), **AI Orchestration**, **Integration / Channels** — hidup di repo terpisah.
>
...
| **[docs/decisions/](./docs/decisions/)** | Audit keputusan | Architecture Decision Records (ADR-0008 ratifies Auth scope) |
| **[docs/SERVICE-CHARTER.md](./docs/SERVICE-CHARTER.md)** | Semua | Auth service bounded context, owns/doesn't-own table, slot routing (supersedes KICKOFF §1 defaults) |
| **⭐ [docs/spec/MVP-AUTH-FIRST.md](./docs/spec/MVP-AUTH-FIRST.md)** | Backend dev (Auth-first slice) | **CANONICAL IMPLEMENTATION SCOPE** — 10-row endpoint table + DB migration order + 7 correctness checks + AC + hand-off checklist |
| **[docs/spec/01-auth-identity.md](./docs/spec/01-auth-identity.md)** | Backend dev | Full Auth surface (5 sections of endpoints) + DDL + RBAC + tenant-guard pseudocode |
| **[docs/spec/API-CONTRACT.md](./docs/spec/API-CONTRACT.md)** | Backend dev | Canonical REST contract (synced dari FE handover) — match shapes exactly |
| **[docs/spec/data-model.md](./docs/spec/data-model.md)** | Backend dev | Cross-service ERD + table ownership matrix (hotels + tiers di Auth per H11) |
| **[docs/spec/open-questions.md](./docs/spec/open-questions.md)** | Backend dev + PM | Q-CONTRACT-07..24 register (Q-22/23/24 + Q-OPS-01 PO-ratified H11) |
...
| Slot | Nama   | PM file (per-dev)                | Default domain (per `docs/SERVICE-CHARTER.md §3`)                          |
| ---- | ------ | -------------------------------- | -------------------------------------------------------------------------- |
| A    | Nathan | [PM-STATUS-A.md](./PM-STATUS-A.md) | Foundation: core, plugins, migrations, tenant-guard, seed-super-admin CLI |
| B    | Nanak  | [PM-STATUS-B.md](./PM-STATUS-B.md) | Auth core: `/api/auth/*`, sessions, `/api/users` (gm_admin scope)         |
| C    | Satrio | [PM-STATUS-C.md](./PM-STATUS-C.md) | Auth admin: `/api/admin/users`, `/api/admin/hotels`, `/api/admin/tiers`   |
